                ...
                <div class="col-span-2 border-t pt-6">
                    ...
                    <div id="tiers-container" class="space-y-4">
                        @foreach($tierRows as $index => $tier)
                            ...
                            @endphp
                            <div class="tier-row p-4 rounded-xl border border-slate-100 bg-slate-50/50 flex gap-3 items-start">
                                ...
                                <div class="flex-1 grid grid-cols-1 md:grid-cols-6 gap-3">
                                    <div class="md:col-span-2">
                                        ...
                                        <input type="text" data-name="name" name="tiers[{{ $index }}][name]" value="{{ $tierName }}" class="w-full px-3 py-2 rounded-lg border border-slate-200 bg-white" required>
                                    </div>
                                    <div class="md:col-span-1">
                                        <label class="block text-[11px] font-bold text-slate-600 mb-1">Harga</label>
                                        <input type="number" data-name="price" name="tiers[{{ $index }}][price]" value="{{ $tierPrice }}" class="w-full px-3 py-2 rounded-lg border border-slate-200 bg-white" required>
                                    </div>
                                    <div class="md:col-span-1">
                                        <label class="block text-[11px] font-bold text-slate-600 mb-1">Stok</label>
                                        <input type="number" data-name="stock" name="tiers[{{ $index }}][stock]" value="{{ $tierStock }}" class="w-full px-3 py-2 rounded-lg border border-slate-200 bg-white">
                                    </div>
                                    <div class="md:col-span-1">
                                        <label class="block text-[11px] font-bold text-slate-600 mb-1">Start</label>
                                        <input type="date" data-name="start_at" name="tiers[{{ $index }}][start_at]" value="{{ $tierStartAtValue }}" class="w-full px-3 py-2 rounded-lg border border-slate-200 bg-white">
                                    </div>
                                    <div class="md:col-span-1">
                                        <label class="block text-[11px] font-bold text-slate-600 mb-1">End</label>
                                        <input type="date" data-name="end_at" name="tiers[{{ $index }}][end_at]" value="{{ $tierEndAtValue }}" class="w-full px-3 py-2 rounded-lg border border-slate-200 bg-white">
                                    </div>
                                </div>
                                <div>
                                    <button type="button" class="btn-remove-tier px-3 py-2 bg-rose-50 text-rose-600 font-bold rounded-lg hover:bg-rose-100 text-xs">Hapus</button>
                                </div>
                            </div>
                        @endforeach
                    </div>
                    ...
                    <div class="mt-3">
                        <button type="button" id="btn-add-tier" class="px-4 py-2 bg-indigo-600 text-white rounded-xl font-bold text-sm hover:bg-indigo-700 transition">+ Tambah Tier</button>
                        <p class="text-xs text-slate-400 mt-2">Atur tanggal mulai & selesai untuk tiap tier. Kosongkan tanggal untuk selalu aktif.</p>
                    </div>
                    ...
                    <template id="tier-template">
                        <div class="tier-row p-4 rounded-xl border border-slate-100 bg-slate-50/50 flex gap-3 items-start">
                            ...
                            <div class="flex-1 grid grid-cols-1 md:grid-cols-6 gap-3">
                                <div class="md:col-span-2">
                                    <label class="block text-[11px] font-bold text-slate-600 mb-1">Nama Tier</label>
                                    <input type="text" data-name="name" name="tiers[__INDEX__][name]" value="" class="w-full px-3 py-2 rounded-lg border border-slate-200 bg-white" required>
                                </div>
                                <div class="md:col-span-1">
                                    <label class="block text-[11px] font-bold text-slate-600 mb-1">Harga</label>
                                    <input type="number" data-name="price" name="tiers[__INDEX__][price]" value="0" class="w-full px-3 py-2 rounded-lg border border-slate-200 bg-white" required>
                                </div>
                                <div class="md:col-span-1">
                                    <label class="block text-[11px] font-bold text-slate-600 mb-1">Stok</label>
                                    <input type="number" data-name="stock" name="tiers[__INDEX__][stock]" value="" class="w-full px-3 py-2 rounded-lg border border-slate-200 bg-white">
                                </div>
                                <div class="md:col-span-1">
                                    <label class="block text-[11px] font-bold text-slate-600 mb-1">Start</label>
                                    <input type="date" data-name="start_at" name="tiers[__INDEX__][start_at]" value="" class="w-full px-3 py-2 rounded-lg border border-slate-200 bg-white">
                                </div>
                                <div class="md:col-span-1">
                                    <label class="block text-[11px] font-bold text-slate-600 mb-1">End</label>
                                    <input type="date" data-name="end_at" name="tiers[__INDEX__][end_at]" value="" class="w-full px-3 py-2 rounded-lg border border-slate-200 bg-white">
                                </div>
                            </div>
                            <div>
                                <button type="button" class="btn-remove-tier px-3 py-2 bg-rose-50 text-rose-600 font-bold rounded-lg hover:bg-rose-100 text-xs">Hapus</button>
